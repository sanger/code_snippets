...
# = Snippet Name
#
# Provides a range of tools for dealing with well names and sorting.
#
# == formatted_range
# Converts an array of well names into a series of well ranges. There are several
# other intermediate methods that may also be useful. Documentation for other
# methods inline
#
# = Example
#
# ```ruby
# WellHelpers.formatted_range(['A1', 'B1', 'C1','A2','A5','B5'])
#   # => 'A1-C1,A2,A5-B5'
# ```
#
...
module WellHelpers
  ...
  def self.formatted_range(wells, size = 96)
    sort_in_column_order(wells).slice_when do |previous_well, next_well|
      index_of(next_well, size) - index_of(previous_well, size) > 1
    end.map { |range| [range.first, range.last].uniq.join('-') }.join(', ')
  end
